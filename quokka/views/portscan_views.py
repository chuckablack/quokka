...
def portscan_store():
    ...
    if not portscan_info:
        return "Must provide portscan information in JSON body", 404
    if "source" not in portscan_info:
        return "Must provide 'source' in portscan information", 404
    if "serial" not in portscan_info:
        return "Must provide 'serial' in portscan information", 404
    if "host_ip" not in portscan_info:
        return "Must provide 'host_ip' in portscan information", 404
    if "host_name" not in portscan_info:
        return "Must provide 'host_name' in portscan information", 404
    if "timestamp" not in portscan_info:
        return "Must provide 'timestamp' in portscan information", 404
    if "scan_output" not in portscan_info:
        return "Must include 'scan_output' in portscan information", 404
